...
9. Update the CI workflow (the file `.github/workflows/main.yml`). The starter code has the Heroku deploy commented out: uncomment it. If you see the line `if: github.ref == 'refs/heads/master'` in the file, update it to match as below (`refs/heads/main`). 
...
## Set up Netlify
**Note:** Everyone should have received an invitation to netfify. Be sure to create your account using your @northeastern.edu email (which might require you to "signup" for a Netlify account with that email instead of sign in with GitHub).
